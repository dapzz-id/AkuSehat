...
@section('content')
    <div class="space-y-4 md:space-y-6">
        <div class="flex items-center gap-2 md:gap-3">
            ...
            <div>
                <h2 class="text-xl md:text-2xl font-bold text-gray-900">Tambah Users Baru</h2>
                <p class="text-xs md:text-sm text-gray-600">Masukkan data users baru</p>
            </div>
        </div>

        <div class="bg-white shadow-sm rounded-lg">
            <form action="{{ route('admin.users.store') }}" method="POST" class="p-4 md:p-6 space-y-4 md:space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    ...
                    <div>
                        ...
                        <select id="divisi-select" name="id_divisi"
                            ...
                            @foreach ($divisi as $k)
                                <option value="{{ $k->id }}" {{ old('id_divisi') == $k->id ? 'selected' : '' }}>
                                    {{ $k->divisi_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="peran-select" name="level"
                            ...
                            <option value="Health Consultant" {{ old('level') == 'Health Consultant' ? 'selected' : '' }}>
                                Health Consultant
                            </option>
                            <option value="Health Monitor" {{ old('level') == 'Health Monitor' ? 'selected' : '' }}>
                                Health Monitor
                            </option>
                            <option value="Member" {{ old('level') == 'Member' ? 'selected' : '' }}>
                                Member
                            </option>
                        </select>
                        @error('level')
                        ...
                        <div>
                            ...
                            <div class="text-xs md:text-sm text-blue-700">
                                <ul class="list-disc list-inside space-y-0.5 md:space-y-1">
                                    <li>Nomor Induk dan Username harus unik (tidak boleh sama dengan users lain)</li>
                                    <li>Password akan di-enkripsi otomatis</li>
                                    <li>Users dapat login menggunakan username dan password ini</li>
